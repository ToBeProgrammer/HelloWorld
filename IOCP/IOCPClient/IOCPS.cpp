...
BOOL IOCPS::Startup()
{
	...
	if ( !m_bServerStarted )
	{
		...
		*/
		...
		m_ContextMap.InitHashTable(m_iMaxNumConnections);  //指定哈希表大小
...
{	
	...
	m_ContextMapLock.Lock(); // Must lock the m_ContextMapLock Protect (m_NumberOfActiveConnections) 
	...
	{		
		...
		CancelIo((HANDLE) clientSocket);//取消由给定句柄所标识的线程添加到队列中的所有IO请求（除非该句柄具有与之相关联的IO完成端口）
		closesocket( clientSocket );
