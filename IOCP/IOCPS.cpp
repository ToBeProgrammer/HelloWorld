...
BOOL IOCPS::Startup()
{
	...
	if ( !m_bServerStarted )
	{
		...
		*  Performance gain to have multiple pending reads if we use more than one IO-worker.
		*  The throughput is going up because of serveral pending reads, but is going down because of 
		*  the ordering part. 
		*/
		...
		// Start the logical Workers. (SetWorkes can be called in runtime..). 
...
{
	IOCPS* pThis = reinterpret_cast<IOCPS*>(pParam);  //传入的是this指针
	if ( pThis )
	{
		while ( !pThis->m_bShutDown )
		{
			...
			int nRet = WSAEnumNetworkEvents(pThis->m_socListen,
				pThis->m_hEvent,
				&events);
			//discovers occurrences of network events for the indicated socket, clear internal network 
...
{	
	...
	{
		// Disconnect. 	
		LINGER lingerStruct;    //The linger structure maintains information about a specific socket that specifies how that 
		                        //socket should behave when data is queued to be sent and the closesocket function is called on the socket.
...
This Bug was a very difficult one.. When I stressed this server code the
server hung after a while. I first thougt that this was a mem-leak problem or 
...
/*
*	Called when a write is completed, this function is often used
*  for progress bars etc (e.g indicates how much is send in bytes) 
*/
...
in the buffer, therefore the need to split the buffer to get a complete message. This is done by the SplitBuffer
*/
...
*sometimes we need to copy information between the buffer, and this is done by the AddAndFlush
*/
...
BOOL IOCPS::SetupListner()
{
	...
	m_hEvent = WSACreateEvent();//This function is used to create an event object that is manually reset with an initial non-signaled state.
	                           //Winsock event objects are system objects in Windows Embedded CE environments. Therefore, if a Windows Embedded CE 
	                           //application requires auto reset events, it can call the native WSACreateEvent function directly. The scope of an 
	...
	int nRet = WSAEventSelect(m_socListen,
		m_hEvent,
		FD_ACCEPT);   //This function specifies an event object to be associated with the supplied set of FD_XXX network events.
	if ( nRet == SOCKET_ERROR )
	{
		CString msg;
...
The SYN flooding attack protection feature of TCP detects symptoms of SYN flooding and responds by reducing the time server spends 
on connection requests that it cannot acknowledge.Specifically, TCP shortens the required interval between SYN-ACK (connection request acknowledgements) 
retransmissions. (TCP retransmits SYN-ACKS when they are not answered.) As a result, the allotted number of retransmissions is consumed quicker and 
the unacknowledgeable connection request is discarded faster.
...
Determines how many connection requests the system can refuse before TCP/IP initiates SYN flooding attack protection. The system must refuse all connection 
requests when its reserve of open connection ports runs out.

TcpMaxConnectResponseRetransmissions - default value is "3"

Determines how many times TCP retransmits an unanswered SYN-ACK (connection request acknowledgment). TCP retransmits acknowledgments until they are answered 
or until this value expires. This entry is designed to minimize the effect of denial-of-service attacks (also known as SYN flooding) on the server.
