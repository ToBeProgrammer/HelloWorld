...
//The most important part of the client or server code is the virtual function NotifyReceivedPackage
//The function handles an incoming message and performs the request sent by the remote connection. In this case, 
//it is only a matter of a simple echo or file transfer. 
//The source code is divided into two projects, IOCP and IOCPClient, which are the server and the client side of the connection.
...
BOOL MyIOCP::BuildPackageAndSend(int ClientID, CString sText)
{
	BOOL bRet=FALSE;
	/*
	//原来代码如下，网友发现有问题，提出以下的修改方式，解决方法得到代码原作者认可
// 	m_ContextMapLock.Lock();
