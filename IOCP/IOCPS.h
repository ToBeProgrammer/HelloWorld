...
public: 
	...
	CCriticalSection m_ContextMapLock;//Holds all the client data (socket, client data, etc.).
	ContextMap m_ContextMap;

	// return the Key (The hostnamn+ socketnr) of a socket. (must be Unique). 
	CString GetHostKeyName(SOCKET socket);	
protected:
	volatile int m_NumberOfActiveConnections;//Holds the number of connected connections
